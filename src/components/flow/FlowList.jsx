...
export default class FlowList extends React.PureComponent {
  ...
  _updateIfNeeded = () => {
    ...
    if (!itemHeights.includes(null) && !itemDidUpdates.some((o, index) => (o === null && this._itemRefs[Math.floor(index / numColumns)])) && itemDidUpdates.includes(2)) {
  ...
  render() {
    ...
    return (
      <FlatList
        ...
        onEndReached={info => {
          if (!JSON.parse(JSON.stringify(this._itemHeights)).includes(null))
            ...
            <FlowListItem
              ...
              onItemHeightChange={(height, index) => {
                ...
                if (this._itemHeights[index] !== height) {
                  const preAllLoaded = !JSON.parse(JSON.stringify(this._itemHeights)).includes(null)
                  this._itemHeights[index] = height
                  const allLoaded = !JSON.parse(JSON.stringify(this._itemHeights)).includes(null)
                  if (allLoaded) {
                    if (preAllLoaded) {
